...
async fn browser_live_smoke_basic_page() {
    ...
    let result = browser_fetch(&lifecycle, "https://example.com", &config).await;
    match result {
        Ok(response) => {
            assert_eq!(
                response.transport,
            ...
            assert!(response.final_url.contains("example.com"));
        }
        Err(e) => {
...
async fn browser_live_smoke_blocks_private_network() {
    ...
    let result = browser_fetch(&lifecycle, "http://127.0.0.1/", &config).await;
