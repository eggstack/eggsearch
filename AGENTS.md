...
`plans/` contains the historical phases plus any explicitly registered active corrective/maintenance work (see `plans/registry.md`). Phases 1-26 are implemented; Phase 27 is the active maintenance/closure housekeeping pass for the optional Eggress 1.0.8 integration. Phase 25's Outcome B architecture remains the baseline: source-build `egress` opt-in for provider upstreams only, with dynamic SSRF-pinned fetch paths direct. Phase 27 is non-functional and is limited to clean-tree release-gate evidence plus qualification-matrix/trigger hardening. Do not create unregistered phase work; new plans must follow the registry/closure convention and the extension rules in `architecture/maintenance.md`.
