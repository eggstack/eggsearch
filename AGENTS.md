...
**Critical: Integration/corpus tests require `--features mock`.** Running `cargo test` without features misses most integration tests. `--all-features` includes `mock`, `pdf`, and `browser`. Scale: 4,963 tests pass with `--all-features` (21 ignored live-smoke); 4,721 with `--features mock` alone. Full suite takes under 2 minutes. Per-suite inventory lives in `docs/test-inventory.md`.
...
src/
  ...
  meta/            # MetadataSearchAdapter + 35 vendored engines (+ local workspace backend) covering 36 registered provider IDs, forge adapter, planners, inventory cache
