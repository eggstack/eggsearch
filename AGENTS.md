...
`plans/` follows the CodeGG-derived planning convention (`plans/003-planning-process.md`, index `plans/README.md`): canonical long-term documents `000`–`003`, ADRs in `adrs/`, subsystem roadmaps in `subsystems/`, milestone handoffs in `implementation/`, evidence gates in `closure/`, traceability in `archive/`, and the compact control surface `plans/registry.md`. Status vocabulary is `proposed`/`ready`/`active`/`blocked`/`closing`/`closed`/`conditionally closed`/`superseded`/`archived`. Phases 1–28 are closed and archived (`plans/archive/phase-*.md` with per-subsystem closure records in `plans/closure/`); the optional Eggress 1.0.8 workstream is closed with no egress-enabled binary published. The active workstream is MCP tool-surface consolidation (roadmap `plans/subsystems/tool-surface-consolidation-roadmap.md`, entry milestone M001 ready). Phase 27 (`6414a72`, hardened qualify run `35810222447`) remains the terminal maintenance/qualification closure baseline; Phase 25's Outcome B architecture remains the runtime baseline. Do not create unregistered work; new plans must follow the registry/closure convention and the extension rules in `architecture/maintenance.md`. Corrective work is a new plan, never a silent amendment.
