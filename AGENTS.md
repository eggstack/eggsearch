...
- **Bounded git execution:** `run_bounded_command()` drains stdout and stderr concurrently with independent capped reads, creates a new process group via `setsid()`, and kills the process group on timeout. Untracked file counts derive from bounded `git ls-files -z --others` output. `git status`, `git check-ignore`, and other auxiliary Git commands also use bounded execution.
