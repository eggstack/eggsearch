...
`plans/` contains the historical phases plus any explicitly registered active corrective/maintenance work (see `plans/registry.md`). Phases 1-27 are implemented; Phase 28 is the active documentation/test-hygiene cleanup for the closed optional Eggress 1.0.8 workstream. Phase 27 remains the terminal maintenance/qualification closure baseline (`6414a72`, hardened qualify run `35810222447`). Phase 28 is non-functional: stale planning-status cleanup plus decoupling host-syntax tests from the optional `egress` feature gate. Phase 25's Outcome B architecture remains the runtime baseline. Do not create unregistered phase work; new plans must follow the registry/closure convention and the extension rules in `architecture/maintenance.md`.
