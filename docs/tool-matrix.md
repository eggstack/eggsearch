...
| `batch_fetch` | Bounded batch fetch over explicit URLs or repo locators | `items`, optional `max_chars`, `timeout_ms`. Web items accept per-item `cache_policy` and `max_cache_age_seconds`; per-item `focus` is deferred to `web_fetch` | `BatchFetchResponse` with per-item results | `external_untrusted` or `local_trusted` | Fetch several known targets in one call |
