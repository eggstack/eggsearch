...
| `provider_status` | Diagnostic report of provider config, health, capabilities, and workflow recipes | none required; optional `recipe_detail` (`none`, `summary`, `full`) | Provider list with `routable`/`skip_reason` fields, `code_hosts`, `health`, `server_capabilities`, `tool_capabilities`, `workflow_recipes` | `local_trusted` | Discover what is actually available before choosing a path |
...
- Each provider in `provider_status` includes `routable` (bool) and `skip_reason` (optional string) indicating whether it can actually be queried.
