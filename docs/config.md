...
When enabled, the local workspace inventory is built automatically on first search (auto-build on cache miss). The git fast path uses `git ls-files -z --cached --others --exclude-standard` with a bounded command runner (5s timeout, 16MB stdout / 64KB stderr caps, sequential pipe reads, post-read truncation, kill-on-timeout watchdog thread). A `git status --porcelain=v2` hash is stored alongside the inventory for change detection between builds. Native directory walking is the fallback for non-git directories.
