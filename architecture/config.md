...
**API providers** (`api` map): `brave_api`, `exa`, `tavily`, `github_code`, `gitlab_code`, `gitea_code`, `sourcegraph`, `semantic_scholar`, `github_advisory`, `osv`, `nvd`, `rustsec`, `cisa_kev` — configured with `enabled`, `api_key_env`, and optional `base_url`. `exa` uses `EXA_API_KEY` and defaults to `https://api.exa.ai/search`. `tavily` uses `TAVILY_API_KEY` and defaults to `https://api.tavily.com/search`.
