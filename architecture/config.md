...
**API providers** (`api` map, `API_PROVIDER_IDS`): `brave_api`, `exa`, `tavily`, `github_code`, `github_issues`, `github_releases`, `gitlab_code`, `gitlab_issues`, `gitlab_releases`, `gitea_code`, `gitea_issues`, `gitea_releases`, `github_advisory`, `semantic_scholar`, `sourcegraph` — configured with `enabled`, `api_key_env`, and optional `base_url`. `exa` uses `EXA_API_KEY` and defaults to `https://api.exa.ai/search`. `tavily` uses `TAVILY_API_KEY` and defaults to `https://api.tavily.com/search`. Advisory providers `osv`, `nvd`, `rustsec`, and `cisa_kev` are keyless, not API-key providers.
