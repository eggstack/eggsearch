...
| Dependency parsing | `src/meta/dependency_parse/` (`mod` owns normalized `parse_dependency_file` dispatch, the `DependencyParseReport` seam, and path-aware basename extraction; `cargo`/`npm`/`go`/`python`/`ruby`/`composer`/`maven`/`dotnet`/`containers`/`github_actions` own one ecosystem each) | Path/size/root validation, applicability policy |
