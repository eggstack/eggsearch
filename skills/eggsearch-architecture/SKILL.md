...
- `commands/` — subcommands: doctor, search, mcp, providers, fetch, update, integrate, plus `browser_login`/`browser_profiles` (both `browser`-feature-gated). Persistent supervision (`startup`, `restart`, `croncheck`) lives in `main.rs` backed by `startup.rs`, not `commands/`
