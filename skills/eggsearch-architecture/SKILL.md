...
- `meta/` — MetadataSearchAdapter + 35 vendored engine structs (+ local workspace backend) covering 36 registered provider IDs, forge adapter, inventory cache
...
36 registered providers (`KNOWN_PROVIDER_IDS` in `src/core/provider.rs`) across 4 search profiles:
