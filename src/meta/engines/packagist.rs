...
pub async fn search(
    ...
    let (name, version) = parse_packagist_query(query);
    if name.is_empty() {
        return Ok(Vec::new());
    }
...
fn parse_packagist_query(query: &str) -> (String, Option<String>) {
...
fn resolution_to_result(resolution: &crate::core::package::PackageResolution) -> SearchResult {
    ...
    SearchResult {
        ...
        source_engine: "packagist".to_string(),
...
mod tests {
    ...
    #[test]
    fn parse_packagist_query_simple() {
        let (name, version) = parse_packagist_query("monolog/monolog");
        assert_eq!(name, "monolog/monolog");
        assert!(version.is_none());
    }

    #[test]
    fn parse_packagist_query_with_version() {
        let (name, version) = parse_packagist_query("monolog/monolog version:3.5.0");
        assert_eq!(name, "monolog/monolog");
        assert_eq!(version.as_deref(), Some("3.5.0"));
    }

    #[test]
    fn parse_packagist_query_with_at_sign() {
        let (name, version) = parse_packagist_query("monolog/monolog@3.5.0");
        assert_eq!(name, "monolog/monolog");
        assert_eq!(version.as_deref(), Some("3.5.0"));
    }

    #[test]
    fn parse_packagist_query_empty() {
        let (name, version) = parse_packagist_query("");
