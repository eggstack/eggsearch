...
pub async fn search(
    ...
    let (name, version) = parse_rubygems_query(query);
    if name.is_empty() {
        return Ok(Vec::new());
    }
...
fn parse_rubygems_query(query: &str) -> (String, Option<String>) {
...
fn resolution_to_result(resolution: &crate::core::package::PackageResolution) -> SearchResult {
    ...
    SearchResult {
        ...
        source_engine: "rubygems".to_string(),
...
mod tests {
    ...
    #[test]
    fn parse_rubygems_query_simple() {
        let (name, version) = parse_rubygems_query("rails");
        assert_eq!(name, "rails");
        assert!(version.is_none());
    }

    #[test]
    fn parse_rubygems_query_with_version() {
        let (name, version) = parse_rubygems_query("rails version:7.1.2");
        assert_eq!(name, "rails");
        assert_eq!(version.as_deref(), Some("7.1.2"));
    }

    #[test]
    fn parse_rubygems_query_with_at_sign() {
        let (name, version) = parse_rubygems_query("rails@7.1.2");
        assert_eq!(name, "rails");
        assert_eq!(version.as_deref(), Some("7.1.2"));
    }

    #[test]
    fn parse_rubygems_query_empty() {
        let (name, version) = parse_rubygems_query("");
