...
pub async fn search(
    ...
    let (name, version) = parse_go_query(query);
    if name.is_empty() {
        return Ok(Vec::new());
    }
...
fn parse_go_query(query: &str) -> (String, Option<String>) {
...
fn resolution_to_result(resolution: &crate::core::package::PackageResolution) -> SearchResult {
    ...
    let url = resolution
        .registry_url
        .clone()
        .unwrap_or_else(|| format!("https://pkg.go.dev/{name}"));
    ...
    SearchResult {
        ...
        source_engine: "go_pkg".to_string(),
...
mod tests {
    ...
    #[test]
    fn parse_go_query_simple() {
        let (name, version) = parse_go_query("github.com/tokio-rs/axum");
        assert_eq!(name, "github.com/tokio-rs/axum");
        assert!(version.is_none());
    }

    #[test]
    fn parse_go_query_with_version() {
        let (name, version) = parse_go_query("github.com/tokio-rs/axum version:v0.7.0");
        assert_eq!(name, "github.com/tokio-rs/axum");
        assert_eq!(version.as_deref(), Some("v0.7.0"));
    }

    #[test]
    fn parse_go_query_with_at_sign() {
        let (name, version) = parse_go_query("github.com/tokio-rs/axum@v0.7.0");
        assert_eq!(name, "github.com/tokio-rs/axum");
        assert_eq!(version.as_deref(), Some("v0.7.0"));
    }

    #[test]
    fn parse_go_query_empty() {
        let (name, version) = parse_go_query("");
