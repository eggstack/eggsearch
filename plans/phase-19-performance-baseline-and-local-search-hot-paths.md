...
Implemented in performance candidate `5a8822ba538e89f9b8f441a328925fab78300754`. The warm search path now clones cached inventory handles rather than snapshots, rebuilds one `Arc<WorkspaceInventory>` shared by cache and search, normalizes path hints once, and uses a deterministic score-once bounded selector shared by warm and rebuilt inventory paths. Public `get_or_build_inventory()` remains owned-return compatible.

Evidence on Rust 1.98.1 / x86_64-apple-darwin: the historical score-only microbenchmarks were 72.7 µs at 1,000 entries and 292.7 µs at 4,096 entries. Those values are not a direct before/after selector comparison. Phase 23 added identical-workload legacy full-sort references at 1.315 ms and 6.452 ms, compared with optimized selector medians of 101.45 µs and 419.26 µs at the same sizes and `max_results=10`. Warm shared-handle characterization measured 4.45 ns. The selector reference-equivalence test, local backend regression tests, static guards, and full gates passed. No lowercase path storage or config/root Arc refactor was added; both were reviewed and deferred as unnecessary after score-once selection.
